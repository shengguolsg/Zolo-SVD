!
  SUBROUTINE ELPA_PDSYEVD( UPLO,na,nev,A,LDA,LCA,EV,Z,nblk,sc_desc,np_rows,&
              np_cols, my_prow, &
              my_pcol,myid, mpi_comm_all,info ) 
!
    use ELPA1
    use ELPA2
    use test_util
...
! .. Scalar Arguments ..
    ...
    INTEGER, INTENT(IN)    :: na,nev,LDA,LCA,nblk,my_prow,my_pcol,myid,mpi_comm_all,&
                              np_rows,np_cols
...
!  write an equivalent interface to pzheevx, and therefore we can easily replace
!  pzheevx with elpa routine. 
!
!  Note that we need to initialize MPI communicator before calling elpa routines 
!  since elpa needs the row and column MPI communicators. 
!
! ..Parameters..
! ===============
!
...
!
!  LDA         Leading dimension of A, local dimension. A, B and Z have the same
!              leading dimension. 
!
...
!
!  sc_desc     An scalapack descriptor, an integer array with dimension(9), desribe 
...
!
!  REAL_KERNEL
!              Optional parameter, choosing a real kernel for 2-stage method
!
! =================================================================
! Written by Shengguo Li, in Aug. 19, 2014
! Another interface of solve_evp_real_2stage for GEP
! 
! =================================================================
!
! .. Local Scalars ..
...
!  ELPA solver needs a full matrix. If the input matrix A is triangular form,
...
!   call mpi_barrier(mpi_comm_all, mpierr) ! for correct timings only
   tt0 = MPI_Wtime()
   !success = solve_evp_real_1stage(na,nev,a,na_rows,ev,z,na_rows,nblk,na_cols, &
   !              mpi_comm_rows, mpi_comm_cols )
   !tt1 = MPI_Wtime()
   !if (myid == 0 ) write(*,*) 'ELPA solver costs time ', tt1 - tt0
   ...
   tt1 = MPI_Wtime()
   if (myid == 0 ) write(*,*) 'Total time costs ', tt1 - tt0
!
!   if (.not.(success)) then
!      write(*,*) "solve_evp_real_2stage produced an error! Aborting..."
