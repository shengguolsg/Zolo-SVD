*
*     Modified by Shengguo Li, NUDT, China, 2016
*
*     ===================
      SUBROUTINE MPDGEQRF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $                    INFO )
*
...
*  =======
*
*  MPDGEQRF computes a QR factorization of a real distributed M-by-N
*  matrix sub( A ) = A(IA:IA+M-1,JA:JA+N-1) = Q * R, where M=2*N, and
*  A = [B; I], the bottom part is an identity matrix, B is a square 
*  matrix. This routine is designed for accelerating the QDWH algorithm. 
*  By taking the special structure of A, we use structured Householder
*  transform to save nearly 60% of floating-point operations.   
*
*  Notes
*  =====
*
...
*          of the distributed submatrix sub( A ). M = 2*N.
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'MPDGEQRF', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
*     JN is the block size, NB, or K if K < NB. 
...
*
      MT = N + JB
      CALL PDGEQR2( MT, JB, A, IA, JA, DESCA, TAU, WORK, LWORK, IINFO )
*
...
*        H = H(ja) H(ja+1) . . . H(jn)
*
         CALL PDLARFT( 'Forward','Columnwise',MT,JB,A,IA,JA,DESCA,
     $                 TAU, WORK, WORK( IPW ) )
*
*        Apply H' to A(ia:ia+m-1,ja+jb:ja+n-1) from the left
*
         CALL PDLARFB( 'Left','Transpose','Forward','Columnwise',MT,
...
*
         MT = MIN( M, J+N+JB-JA )
         CALL PDGEQR2( MT-J+JA, JB, A, I, J, DESCA, TAU, WORK, LWORK,
     $                 IINFO )
*
