...
!-------------------------------------------------------------------------------
   ...
   ! Vectors for storing REAL sparse matrix in Matrix-Market form
   integer :: nbw, nnz
   ...
   real*8, parameter :: ZERO = 0.d0, ONE = 1.d0, MONE = -1.0D+00
...
!   my_blacs_ctxt = mpi_comm_world
   ...
   allocate(ev(na))
 ! Allocate matrices and set up a test matrix for the eigenvalue problem
   ! The symmetric matrix in MM form is only stroed its upper or lower
   ! triangular part.
   ! 
   !filename = 'bcsstk16.mtx'
   !filename = 'linverse.mtx'
   !filename = '../Zolo-SVD/LF10000_M.mtx'
   !filename = '../Zolo-SVD/linverse.mtx'
   !filename = '../Zolo-SVD/nemeth03.mtx'
   !filename = '../Zolo-SVD/fv1.mtx'
   !filename = '../Zolo-SVD/poli_large.mtx'
   filename = '../../ZoloSVD-final/bcsstk18/bcsstk18.mtx'
   !filename = 'toep.mtx'
   CALL PDLASET( 'A',na,na,ZERO,ZERO,A,1,1, sc_desc)
   CALL PDLASET( 'A',na,na,ZERO,ZERO,U,1,1,sc_desc )
   CALL prepare_real_symm_MM( filename,na,nbw,nnz,myid,my_prow,my_pcol, &
        np_rows,np_cols,sc_desc,A,U,SIGMA )

   !CALL prepare_real_GEMM( filename,na,nnz,myid,my_prow,my_pcol, &
   !     np_rows,np_cols,sc_desc,A,U,SIGMA )
   ...
   normA = maxval( ev(1:na) )
   err = errmax
   call mpi_allreduce(err,errmax,1,MPI_REAL8,MPI_MAX,MPI_COMM_WORLD,mpierr)
   if(myid==0) print *
   if(myid==0) print *,'Error Residual     :',errmax, errmax/(na*normA), normA

   !--------------------------------------------------------------
   ! Check the orthogonality of computed singular vectors

   ! tmp1 = U**T * U
   call pdgemm('C','N',na,na,na,ONE,u,1,1,sc_desc, &
               u,1,1,sc_desc,ZERO,A,1,1,sc_desc)
   ! Initialize Z to unit matrix
   call pdlaset('A',na,na,ZERO,ONE,As,1,1,sc_desc )

   ! Q = U**T * U - Unit Matrix, Q = Q -Z
   CALL PDGEADD( 'N',na,na,MONE,As,1,1,sc_desc,ONE,A,1,1,sc_desc )
   ...
   call pdgemm('C','N',na,na,na,ONE,vt,1,1,sc_desc, &
               vt,1,1,sc_desc,ZERO,A,1,1,sc_desc )
   ! Initialize Z to unit matrix
   call pdlaset('A',na,na,ZERO,ONE,As,1,1,sc_desc )
