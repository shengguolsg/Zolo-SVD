...
!  Initialize a single BLACS context
   ...
   call PDGEQRF( Ma, Na, A, 1, 1, sc_desc, ev, rtmp, LWORK, INFO )
   if( info .ne. 0 ) then
   ...
   ttt0 = MPI_Wtime()
   call PDGEQRF( Ma, Na, A, 1, 1, sc_desc, ev, work, LWORK, INFO )
   ttt1 = MPI_Wtime()
   ...
   ttt0 = MPI_Wtime()
   call PDORGQR( Ma, Na, Na, a, 1, 1, sc_desc, ev, WORK, LWORK, INFO )
   ttt1 = MPI_Wtime()
